...
class FidTransformerConfig(PretrainedConfig):
    model_type = "fid_transformer"
...
## copied from MarianPreTrainedModel
class BasePreTrainedModel(PreTrainedModel):
    config_class = FidTransformerConfig
...
## copied from MarianEncoder
class FidTransformerEncoder(BasePreTrainedModel):
    ...
    def __init__(self, config, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if config.positional_embedding_type == 'sinusoidal':
            self.embed_positions = SinusoidalPositionalEmbedding(
                config.max_position_embeddings, embed_dim, self.padding_idx
            )
        elif config.positional_embedding_type == 'learned':
    ...
    def forward(
        ...
        each_len = total_len // self.config.memory_number
        input_ids = input_ids.view(batch_size*self.config.memory_number,each_len)
        attention_mask = attention_mask.view(batch_size*self.config.memory_number,each_len)
        ## FID modification
        ...
        ## FID modification
        hidden_states = hidden_states.view(batch_size, self.config.memory_number*each_len, -1)
        ## FID modification
...
## copied from MarianDecoder
class TransformerDecoder(BasePreTrainedModel):
...
class BaseFidTransformer(BasePreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.encoder = FidTransformerEncoder(config, encoder_embed_tokens)
...
class FidTransformer(BasePreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = BaseFidTransformer(config)
