def restore_order_from_accelerate(all_rank_outputs):
    """
    all_rank_outputs: [
        [[1,2],[5,6]], #rank0 output
        [[3,4],[7,x]], #rank1 output
    ]
    """
    num_ranks = len(all_rank_outputs)
    num_batche_per_rank = len(all_rank_outputs[0])
    # batch_size = len(all_rank_outputs[0][0])
    outputs = []
    for idx in range(num_batche_per_rank):
        for jdx in range(num_ranks):
            outputs.extend(all_rank_outputs[jdx][idx])
    return outputs

def restore_order_from_ddp(all_rank_outputs,world_size):
    """
    restore order from distributed sampler, notice this function couldn't delete padding data
    Input:
        all_rank_outputs: List[ModelOutput]
    Output:
        restored List[ModelOutput]
    """
    num_data_per_rank = int(len(all_rank_outputs)/world_size)
    output = []
