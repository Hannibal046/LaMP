def get_tokenized_rouge(hyps,refs):
    from rouge_metric import PyRouge
    rouge = PyRouge(rouge_n=(1, 2, 3,4), rouge_l=True, rouge_w=False,
...
def get_tokenized_bleu(hyps,refs):
    from nltk.translate.bleu_score import corpus_bleu
    from nltk.translate import bleu_score as nltkbleu
    refs = [[ref] for ref in refs]
    b1 = corpus_bleu(refs, hyps, weights=(1.0/1.0,), smoothing_function=nltkbleu.SmoothingFunction(epsilon=1e-12).method1)
    b2 = corpus_bleu(refs, hyps, weights=(1.0/2.0, 1.0/2.0), smoothing_function=nltkbleu.SmoothingFunction(epsilon=1e-12).method1)
    b3 = corpus_bleu(refs, hyps, weights=(1.0/3.0, 1.0/3.0, 1.0/3.0), smoothing_function=nltkbleu.SmoothingFunction(epsilon=1e-12).method1)
    b4 = corpus_bleu(refs, hyps, weights=(1.0/4.0, 1.0/4.0, 1.0/4.0, 1.0/4.0), smoothing_function=nltkbleu.SmoothingFunction(epsilon=1e-12).method1)
    return (b1, b2, b3, b4)    
...
def get_bleu_score(hyps,refs,return_signature=False,**kwargs):
    ...
    import sacrebleu
    scorer = sacrebleu.metrics.BLEU(force=True,**kwargs)
...
def get_ter_score(hyps,refs,trg_lang=None,return_signature=False):
    
    assert len(hyps) == len(refs)
    import sacrebleu
    if trg_lang == 'zh':
        scorer = sacrebleu.TER(asian_support=True,normalized=True)
    else:
...
def split_chinese_char(s):
    return list(s)

def get_meteor_score(hyps,refs,trg_lang=None):
    ...
    if trg_lang == 'zh':
        tokenizer = split_chinese_char
    elif trg_lang == 'de':
        tokenizer = partial(word_tokenize,language='german')
    elif trg_lang == 'en' or trg_lang is None:
        tokenizer = partial(word_tokenize,language='english')
    scores = []
    for hyp,ref in zip(hyps,refs):
        scores.append(meteor([tokenizer(ref)],tokenizer(hyp)))
